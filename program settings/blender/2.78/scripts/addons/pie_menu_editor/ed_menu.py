...
from .ed_base import (
    ...
    PME_OT_pmi_remove, WM_OT_pmi_icon_select)
...
class WM_OT_rmi_add(bpy.types.Operator):
    ...
    bl_label = "Add Item or Column"
    bl_description = "Add an item or a column"
    bl_options = {'INTERNAL'}
...
class WM_OT_rm_col_specials_call(bpy.types.Operator):
    ...
    def _draw(self, menu, context):
        ...
        lh.operator(
            WM_OT_rmi_add.bl_idname, "Add Item", 'ZOOMIN',
        ...
        lh.operator(
            WM_OT_rm_col_move.bl_idname, "Move Column", 'ARROW_LEFTRIGHT',
...
class WM_OT_rm_col_remove(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.ask:
            context.window_manager.popup_menu(
                self._draw, WM_OT_rm_col_remove.bl_description)
        else:
...
class WM_OT_rmi_specials_call(bpy.types.Operator):
    ...
    def _draw(self, menu, context):
        ...
        lh.operator(
            WM_OT_rmi_move.bl_idname, "Move Item", 'ARROW_LEFTRIGHT',
            pm_item=self.pm_item,
            idx=-1)
        ...
        for pmi in pm.pmis:
            ...
            if pmi.mode == 'EMPTY':
                if not pmi.text:
                    ...
                    lh.row(column)
                    lh.operator(
                        WM_OT_rmi_add.bl_idname, "Add Item",
                        index=idx,
                        mode='ITEM')
                    lh.operator(
