...
from bpy_extras import image_utils, view3d_utils
...
class TF_Add_Transform(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        selection = [seq for seq in context.selected_sequences if seq.select and seq.type not in ['SOUND','TRANSFORM']]
...
class TF_Draw_Selection(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        for seq in reversed(context.selected_sequences):
...
class TF_Select_Layers(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        seq = context.selected_sequences[self.name]
        if not multi :
            bpy.ops.sequencer.select_all(action='DESELECT')
            seq.select = True
        else :
        ...
        bpy.context.scene.sequence_editor.active_strip = context.selected_sequences[self.name]
        bpy.ops.sequencer.tf_draw_selection('INVOKE_DEFAULT')
        return {'FINISHED'}
##########################  Draw 2d cursor ###################################
